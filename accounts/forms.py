...
class RegistrationStoreForm(forms.ModelForm):
    ...
    class Meta:
        model = Account
        fields = ['first_name', 'last_name', 'name_store', 'address', 'phone_number', 'email', 'password']

    def __init__(self, *args, **kwargs):
        super(RegistrationStoreForm, self).__init__(*args,**kwargs)
        self.fields['first_name'].widget.attrs['placeholder']='Ingrese nombre'
        self.fields['last_name'].widget.attrs['placeholder']='Ingrese apellidos'
        self.fields['name_store'].widget.attrs['placeholder']='Ingrese nombre de tu tienda'
        self.fields['address'].widget.attrs['placeholder']='Ingrese direccion de tienda'
        self.fields['phone_number'].widget.attrs['placeholder']='Ingrese telefono'
        self.fields['email'].widget.attrs['placeholder']='Ingrese email'
        for field in self.fields:
            self.fields[field].widget.attrs['class']='form-control'
    ...
    def clean(self):
        cleaned_data = super(RegistrationStoreForm,self).clean()
        password = cleaned_data.get('password')
        confirm_password = cleaned_data.get('confirm_password')
